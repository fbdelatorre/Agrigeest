...
BEGIN
  ...
  IF invitation_code IS NULL OR TRIM(invitation_code) = '' THEN
  ...
  -- Log for debugging
  RAISE NOTICE 'Invitation lookup result: %', invitation_record;
  ...
  IF invitation_record.expires_at < now() THEN
    RAISE NOTICE 'Invitation expired: expires_at=%, now=%', invitation_record.expires_at, now();
    RETURN json_build_object(
      'valid', false,
...
BEGIN
  ...
  BEGIN
    ...
    -- Log for debugging
    RAISE NOTICE 'Processing invitation: %', invitation_record;
    ...
    -- Log user details
    RAISE NOTICE 'User details: %', user_record;
    ...
    RAISE NOTICE 'Profile record: %', profile_record;
  ...
  EXCEPTION
    WHEN OTHERS THEN
      ...
      RAISE NOTICE 'Error in join_institution: %', SQLERRM;
