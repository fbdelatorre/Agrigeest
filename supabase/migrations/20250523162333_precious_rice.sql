-- Drop any existing functions from all schemas
DROP FUNCTION IF EXISTS public.validate_invitation(text);
DROP FUNCTION IF EXISTS auth.validate_invitation(text);
DROP FUNCTION IF EXISTS storage.validate_invitation(text);
DROP FUNCTION IF EXISTS realtime.validate_invitation(text);
DROP FUNCTION IF EXISTS public.join_institution(uuid, text);

-- Create function to validate invitation with proper schema access
...
BEGIN
  ...
  -- Log for debugging
  RAISE LOG 'Invitation lookup result: %', invitation_record;
...
-- Create function to join institution with proper schema access
...
BEGIN
  ...
  BEGIN
    ...
    -- Log for debugging
    RAISE LOG 'Processing invitation: %', invitation_record;
    ...
    RAISE LOG 'Profile created/updated: %', profile_record;
