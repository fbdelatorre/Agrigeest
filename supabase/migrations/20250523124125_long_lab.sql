/*
  # Fix invitation system functions
  
  1. Changes
    - Recreate invitation management functions with proper timestamp handling
    - Set strict 4-hour expiration window
    - Improve validation and error messages
    - Update existing invitations
    
  2. Security
    - Maintain SECURITY DEFINER settings
    - Keep strict permission checks
*/

-- Drop existing functions
DROP FUNCTION IF EXISTS create_invitation(uuid);
DROP FUNCTION IF EXISTS validate_invitation(text);
DROP FUNCTION IF EXISTS clean_expired_invitations();

-- Recreate create_invitation function with strict 4 hour expiration
CREATE OR REPLACE FUNCTION create_invitation(
  institution_id_param uuid
)
RETURNS text
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
DECLARE
  new_code text;
BEGIN
  -- Check if user is admin
  IF NOT EXISTS (
    SELECT 1 FROM user_profiles
    WHERE id = auth.uid()
    AND is_admin = true
    AND institution_id = institution_id_param
  ) THEN
    RAISE EXCEPTION 'Only administrators can create invitations';
  END IF;

  -- Generate unique code
  new_code := upper(substring(md5(random()::text) from 1 for 8));

  -- Create invitation with exactly 4 hours expiration
  INSERT INTO invitations (
    institution_id,
    code,
    expires_at,
    created_by
  ) VALUES (
    institution_id_param,
    new_code,
    CURRENT_TIMESTAMP + interval '4 hours',
    auth.uid()
  );

  RETURN new_code;
END;
$$;

-- Recreate validate_invitation function with strict timestamp comparison
CREATE OR REPLACE FUNCTION validate_invitation(
  invitation_code text
) RETURNS TABLE (
  is_valid boolean,
  institution_id uuid,
  message text
) AS $$
BEGIN
  -- Return invalid if invitation doesn't exist
  IF NOT EXISTS (SELECT 1 FROM invitations WHERE code = invitation_code) THEN
    RETURN QUERY SELECT 
      false AS is_valid,
      NULL::uuid AS institution_id,
      'Invalid invitation code'::text AS message;
    RETURN;
  END IF;

  -- Return invalid if invitation is already used
  IF EXISTS (
    SELECT 1 FROM invitations 
    WHERE code = invitation_code 
    AND used_at IS NOT NULL
  ) THEN
    RETURN QUERY SELECT 
      false AS is_valid,
      NULL::uuid AS institution_id,
      'This invitation has already been used'::text AS message;
    RETURN;
  END IF;

  -- Return invalid if invitation is expired
  IF EXISTS (
    SELECT 1 FROM invitations 
    WHERE code = invitation_code 
    AND expires_at <= CURRENT_TIMESTAMP
  ) THEN
    RETURN QUERY SELECT 
      false AS is_valid,
      NULL::uuid AS institution_id,
      'This invitation has expired'::text AS message;
    RETURN;
  END IF;

  -- Return success with institution id
  RETURN QUERY 
    SELECT 
      true AS is_valid,
      institution_id,
      'Valid invitation'::text AS message
    FROM invitations 
    WHERE code = invitation_code;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- Recreate clean_expired_invitations function
CREATE OR REPLACE FUNCTION clean_expired_invitations()
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
BEGIN
  DELETE FROM invitations
  WHERE expires_at <= CURRENT_TIMESTAMP
  AND used_at IS NULL;
END;
$$;

-- Update existing invitations to have correct expiration time
UPDATE invitations 
SET expires_at = created_at + interval '4 hours'
WHERE used_at IS NULL;